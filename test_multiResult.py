        ...
        for i, data in tqdm(enumerate(test_dataloader), total=len(test_dataloader), desc='Generating Samples'):
            ...
            for k in range(multi_sample_num):
                ...
                for j in range(data['edit_sketch'].shape[0]):
                    ...
                    cv2.imwrite(os.path.join(outf_syn,'test/', f'{i}_{k}_y.png'), s.permute(1,2,0).cpu().numpy())
                ## xの点群をnpyファイルで一つ一つ保存
                for j in range(x.shape[0]):
                    np.save(os.path.join(outf_syn,'test/', f'{i}_{k}_x.npy'), x[j].cpu().numpy())
                ## genの点群をnpyファイルで一つ一つ保存
                for j in range(gen.shape[0]):
                    np.save(os.path.join(outf_syn,'test/', f'{i}_{k}_gen.npy'), gen[j].cpu().numpy())
                for j in range(f.shape[0]):
                    np.save(os.path.join(outf_syn,'test/', f'{i}_{k}_f.npy'), f[j].cpu().numpy())
